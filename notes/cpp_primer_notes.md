...
`int i = 1024, *p = &i, &r = i;`，i是int整型，p是int整型指针，r是int整型引用。

###### 定义多个变量

`int* p, q;`这样写容易产生误导，`*`只修饰p，对于后面的变量不起作用。（为了方便理解，我之前常这样写。）

- 把修饰符和变量标识符写在一起，强调变量<u>具有</u>复合类型。`int *p1, *p2;` 
- 把修饰符和类型名称写在一起，并且每条语句只定义一个变量，强调本次声明<u>定义了一种</u>复合类型。`int* p1; int* p2;` 

没有对错，选一个一直坚持即可。本书使用第一种。

###### 指向指针的指针

<u>指针是内存中的对象</u>，像其他对象一样，也有自己的地址。因此允许把指针的地址存放到另一个指针中。

```c++
int ival = 1024;
int *pi = &ival; // int整型ival的指针
int **pii = *pi; // 指针pi的指针
std::cout<< "the value of ival\n"
    	 << "direct value:" << ival << "\n"
    	 << "indirect value:" << *pi << "\n"
    	 << "double indirect value:" << **pii << "\n";
```

###### 指向指针的引用

<u>引用不是对象而是别名</u>，因此不能定义指向引用的指针。
...
r = &i;		// r引用了指针，对r赋值i，就是让指针p指向i
*r = 0;		// *r也就是*p，也就是i，这里是给i赋值0
```

我的理解方法：**把引用完全想成别名，替换成本名即可**

理解r的类型是什么，最简单的方法是，**从右向左阅读r的定义，离变量最近的符号对变量的类型具有最直接的影响**：

- r左边紧挨着是&，因此r是引用值
- 再左边是*，因此引用值r引用的对象是指针
- 最左边是int，引用值r引用的是int类型的指针（指向int类型的变量）



> ```c++
> int* p = 0;	// 正确，定义并初始化p是空指针，不指向任何变量
> int* p = 1; // 错误，不能给指针直接赋常量值，必须指向对象
> 
> int i = 0;
> 
> int* pi = &i;	// 正确，定义并初始化pi指向i
> 
> int *pii;
> *pii = 0;	// 错误，应该是指向变量，而不是常量
> *pii = i;	// 错误，没有初始化的指针不能使用解引用操作
> pii = &i;	// 正确，初始化地址值
> pii = pi;	// 正确，赋地址值（没初始化也可以，相当于初始化）
> ```
>
> 总结：所以建议==初始化所有指针== 。可以理解为：**指针本身（不带\*的）需要初始化，如果没初始化，就不能用\* 解引用**。
...
const对象一旦创建后其值就不能再变化，所以<u>const对象必须初始化</u>：

```c++
const int bufSize = 256;
bufSize = 512; // 错误：const变量不能写值
...
只能在const类型的对象上执行不改变其内容的操作。如const int跟int一样都能参与算术运算，也都能转换成布尔值，等等。

const对象也可以进行初始化操作。如果用一个对象去初始化另一个对象，则它们是不是const都无关紧要：

```C++
int i = 42;
const int ci = i;	// int 给 const int 初始化
