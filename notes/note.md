...
#### 类的成员函数重写（覆盖）、重载、隐藏的区别

- 重写（覆盖）：
  - 直接重写父类的方法，只发生在类的成员函数中
  - 函数名、参数列表、返回类型必须与父类完全相同
  - 重写的父类中，被重写函数必须有virtual标识
...
一般情况下，<u>类中含有指针成员变量时，需要重载拷贝构造函数、赋值运算符、析构函数</u>。
...
- 用于<u>不同类型指针直接、不同类型引用之间、指针和能容纳指针的整数类型之间</u>的转换
...
- 默认的拷贝构造函数和赋值运算符=都是浅拷贝
  - 赋值操作符=只有在生成的代码合法时才会生成
    - 如果向在内含引用成员、指针成员、const成员的类中支持赋值操作，就必须自己定义赋值运算符=
...
- 有赋值运算符=的，都是拷贝初始化，==调用拷贝构造函数==。如果想像 unique_ptr 那样禁止拷贝，将拷贝构造函数私有化即可。
...
- reserve 和 resize
  - reserve 是直接扩充到已经确定的大小，可以减少由 push_back 超过容量造成的多次开辟、转移、释放的问题。只有一个参数
  - resize 可以改变有效空间capacity的大小。可以有多个参数
...
如果需要空间动态缩小，可以考虑使用deque（双端队列、连续空间）
...
list底层是一个==双向链表==，以结点为单位存储数据，结点的地址在内存中不一定连续。每次增/删一个元素，就配置或释放一个元素空间。
...
deque是一个双端队列（双向开口的连续线性空间），在头、尾两端进行元素的插入跟删除操作都有理想的时间复杂度。
...
- 从任意结点到其子树中每个叶子结点的路径都包含相同数量的黑色结点（黑色高度相同）

由这五条性质可以推出：红黑树在最差情况下，最长路径都不比最短路径长2倍
...
<u>（跟平衡二叉树相比，B树每个结点包含的关键字增加，并且所有叶子节点均在同一层。充分利用空间 + 降低并平衡层数）</u> 
...
- set 的迭代器是const的，不允许修改元素的值；map允许修改value，但不允许修改key的值
...
C++ 11标注没有hash_map，但出于编译器扩展的目的一些库将它实现了。
...
- hash_map 是<u>哈希关联容器</u>，使用<u>哈希表</u>实现，操作的时间复杂度是O(1)级别
  - 哈希关联容器是用hash table实现的关联容器，元素不进行排序，最差复杂度O(n)，平均复杂度O(1)
  - 在不要求排序，只要求存取的场合，哈希关联容器的效率远高于排序关联容器
- map是<u>排序关联容器</u>，使用<u>红黑树</u>实现，操作的时间复杂度是O(log(n))级别
...
哈希表的基本思路是空间换时间。大多数情况下查找或者插入元素的时间复杂度可以达到O(1)， 时间主要花在计算hash值上。然而也有一些极端的情况，最坏的就是hash值全都映射在同一个地址上，这样哈希表就会退化成链表，时间复杂度会变为O(n)。
...
- 使用拉链法解决冲突
- bucket维护的若干“拉链”不是由STL list实现，而是使用hashtable_node自定义的linked_list
- bucket本身使用vector存储
...
unordered_map 是一个防冗余的哈希表（哈希函数采用除留余数法，冲突使用拉链法）。用空间换时间，可以在 O(1) 的复杂度读数据。

###### unordered_map 和 map

- unordered_map 自动去重，但是不排序，常用作hash表；而map使用它的自动排序特性

- unordered_map 是哈希表，bucket是vector，每个拉链是一个链表。
  - 空间：哈希表是空间换时间，需要很大的存储空间
  - 查找时间：O(1)
  - 新建、扩容：哈希表构造速度慢（有hash函数的耗时和去冲突的耗时），扩容需要进行重新哈希，十分耗时
...
迭代器是连接算法和容器的桥梁。通过迭代器，可以在不了解容器内部原理的情况下遍历容器。
...
- 使用萃取技术进行推到的过程中，会使用到==模板偏特化==，用来推导参数
...
- 删除操作
  ...
  - 对于list和forward_list（双向链表，不连续）
    - 所有的迭代器、指针、引用有效
  - 对于关联容器map，如果一个元素已经被删除，那么其对应的迭代器就失效了
...
- 内存池：
  - chunk_alloc()函数内管理了一块内存池，当refill函数要填充链表时，就会调用chunk_alloc函数，从内存池
    取出相应的内存
