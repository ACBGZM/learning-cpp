...
int main(){
    ...
    strcpy(mystruct2.str, a);		// 把常量区字符串的值拷贝到类中
...
当编译器发现类中有虚函数时，会创建虚函数表，==把虚函数的入口地址放入虚函数表，并在对象中增加一个vptr，用于指向类的虚函数表==。<u>当子类重写基类的虚函数时，会将虚函数表中对应的指针进行替换，从而调用子类覆盖后的虚函数，实现动态绑定</u>。（==vptr依然指向各自类的虚函数表，虚函数表内部的指针改变==）
